...
export default function LoginPage() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      const { code } = await storeTempCode();
      setCode(code);
    })();
  }, []);
    ...
    const pollUser = async () => {
      ...
      try {
        ...
        if (user && user.success === true) {
          // ! Successfully authenticated, stop polling
          setIsPolling(false);
          ...
          // ? Keep polling every second until we get a successful response
          if (isPolling) {
        ...
        // ! Keep trying even on error ( let him cook! )
        if (isPolling) {
          setTimeout(pollUser, 1000); // ? Also poll every 1 second on error
        }
      }
    };
